...
\begin{figure}[H]
    ...
    \end{tikzpicture}
\end{figure}
We've marked this fan with vectors associated to the first lattice point each ray intersects.
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
\end{figure}
From our definition, we're interested in the rays \( \sigma_1 \setminus \tau = \rho_1 \) and \( \sigma_4 \setminus \tau = \rho_3 \), and if the weighted sum of their associated marked points is in the span of \( \tau \).
\begin{figure}[H]
    \centering
    \newcommand{\GridDim}{2}
    \begin{tikzpicture}[scale=1]
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
\end{figure}
Some inspection will reveal that \( \pCub(\bSigma, \bigcdot) \subseteq \R^3 \) where all coordinates are non-negative.
...
Now we'll fix \( z = (1,2,3,4) \) and note, importantly, that \( z \in \pCub(\bSigma, \bigcdot) \) and \( z \in \pCub(\bSigma, \star) \).
...
We'll adjust the grid to represent the integer lattice given by the dual basis, and we can see that the volumes do agree.
